...
const fullIndexUrl=new URL("../../data/generated/measures-index.json",import.meta.url);
const metaUrl=new URL("../../data/generated/measures-index-meta.json",import.meta.url);
const shardBaseUrl=new URL("../../data/generated/measures-by-chapter/",import.meta.url);
let metaPromise,fullIndexPromise;
const shardPromises=new Map();
const parseJson=url=>readFile(url,"utf8").then(JSON.parse);
const mergeRows=(left={},right={})=>{
  const result={...left};
  for(const[key,rows]of Object.entries(right))result[key]=result[key]?[...result[key],...rows]:rows;
  return result;
};
async function optionalShard(name){
  if(!shardPromises.has(name))shardPromises.set(name,parseJson(new URL(`${name}.json`,shardBaseUrl)).catch(error=>{
    if(error?.code==="ENOENT")return{byCode:{},ratesByCode:{}};
    throw error;
  }));
  return shardPromises.get(name);
}
async function loadMeasuresData(code){
  try{
    metaPromise??=parseJson(metaUrl);
    const meta=await metaPromise;
    const[globalData,chapterData]=await Promise.all([optionalShard(meta.globalShard||"_global"),optionalShard(code.slice(0,2))]);
    return{
      ...meta,
      byCode:mergeRows(globalData.byCode,chapterData.byCode),
      ratesByCode:mergeRows(globalData.ratesByCode,chapterData.ratesByCode),
    };
  }catch(error){
    if(error?.code!=="ENOENT")throw error;
    fullIndexPromise??=parseJson(fullIndexUrl);
    return fullIndexPromise;
  }
}
const rateScopeKey=item=>JSON.stringify([item.type,item.area,item.additionalCode]);
...
export default async request=>{
  ...
  try{
    const data=await loadMeasuresData(code);
    const found=[];
    ...
    const maxRateSpecificity=new Map();
    for(const item of found)if(item.isRate){
      const key=rateScopeKey(item),length=item.sourceCode.length;
      if(length>(maxRateSpecificity.get(key)||0))maxRateSpecificity.set(key,length);
    }
    const scopedFound=found.filter(item=>!item.isRate||item.sourceCode.length===maxRateSpecificity.get(rateScopeKey(item)));
