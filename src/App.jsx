...
export default function App() {
  ...
  const loadCnenRecord=async(codeValue)=>{setCnenLoading(true);setCnenError("");try{const data=await getJson(`/api/cnen-content?code=${encodeURIComponent(codeValue)}`);setCnenSelected(data.record);}catch(error){setCnenSelected(null);setCnenError(error.message);}finally{setCnenLoading(false);}};
  ...
  return <main>
    <nav><div className="brand"><ShieldCheck />TARIC 2026</div><div className="nav-actions"><button className={topPanel==="content"?"active":""} onClick={()=>setTopPanel(topPanel==="content"?null:"content")}>{L("Tartalom","Content")}</button><button onClick={()=>setTopPanel(topPanel==="integration"?null:"integration")}>{L("Integráció","Integration")}</button><button onClick={()=>setTopPanel(topPanel==="settings"?null:"settings")}>{L("Beállítások","Settings")}</button><div className="language"><button className={lang === "hu" ? "active" : ""} onClick={() => setLang("hu")}>HU</button><button className={lang === "en" ? "active" : ""} onClick={() => setLang("en")}>EN</button></div></div></nav>
    {topPanel==="content"&&<section className="top-drawer cnen-browser">
      ...
      {cnenData&&<><div className="cnen-result-meta">{cnenData.query?L(`${cnenData.total} találat`,`${cnenData.total} results`):L("Kétjegyű KN-fejezetek – a négyszámjegyű vámtarifaszámok lenyithatók","Two-digit CN chapters – expand to browse four-digit headings")}</div><div className="cnen-browser-grid">
        ...
        <article className="cnen-detail">{cnenSelected?<><div className="cnen-detail-title"><code>{groupedCnen(cnenSelected.code)}</code><h3>{lang==="hu"&&cnenSelected.headingHu?cnenSelected.headingHu:cnenSelected.heading}</h3></div>{lang==="hu"&&cnenSelected.headingHu&&<p className="cnen-original-heading"><b>EN:</b> {cnenSelected.heading}</p>}<div className="cnen-rule-tags">{cnenSelected.ruleTypes?.map((type)=><span key={type}>{cnenRuleLabel(type)}</span>)}</div><h4>{L("Eredeti angol magyarázó szöveg","Explanatory note")}</h4><p>{cnenSelected.content}</p><button type="button" className="cnen-children" onClick={()=>{setCnenQuery(cnenSelected.code);loadCnenSearch(cnenSelected.code);}}>{L("Kód alatti megjegyzések tallózása","Browse notes below this code")}</button></>:<p>{L("Nyisson le egy kétjegyű fejezetet, majd válasszon vámtarifaszámot.","Expand a two-digit chapter, then select a heading.")}</p>}</article>
      </div></>}</section>}
    ...
    <section className="panel agent-panel"><form className="agent-form" onSubmit={submit}>
      <label>{L("Termék neve *","Product name *")}<input value={product} onChange={(e) => {setProduct(e.target.value);setConfirmedFacts({});}} placeholder={L("pl. lítium-ion akkumulátoros csavarbehajtó","e.g. lithium-ion cordless screwdriver")} /></label>
      <label>{L("Rövid leírás","Short description")}<textarea value={query} onChange={(e) => {setQuery(e.target.value);setConfirmedFacts({});}} onInput={(e)=>{e.currentTarget.style.height="auto";e.currentTarget.style.height=`${Math.min(e.currentTarget.scrollHeight,180)}px`;}} placeholder={L("anyag, funkció, feldolgozottság, kiszerelés — minél pontosabb, annál biztosabb a besorolás","material, function, processing and packaging — more detail improves classification")} rows="1" /></label>
      ...
      <button className={`primary agent-submit ${loading ? "is-loading" : ""}`} disabled={loading || (!code && !product.trim() && !query.trim())}>{loading ? <LoaderCircle className="spin" size={16} /> : <Search size={18} />}{progressLabel}</button>
    </form></section>
