...
export default function App() {
  ...
  return <main>
    ...
    {topPanel==="content"&&<section className="top-drawer cnen-browser"><div className="cnen-browser-heading"><div><h2>{L("KN Magyarázó Megjegyzések","CN Explanatory Notes")}</h2><p>{L("Keresés KN-kód, megnevezés vagy a magyarázat szövege alapján.","Search by CN code, heading or explanatory text.")}</p></div>{cnenData&&<span>{Number(cnenData.recordCount).toLocaleString(lang==="hu"?"hu-HU":"en-GB")} {L("kódolt megjegyzés","coded notes")}</span>}</div><form className="cnen-search" onSubmit={(event)=>{event.preventDefault();loadCnenSearch();}}><input value={cnenQuery} onChange={(event)=>setCnenQuery(event.target.value)} placeholder={L("KN-kód vagy kifejezés, pl. 4202, mobile phones…","CN code or phrase, e.g. 4202, mobile phones…")}/><button type="submit" disabled={cnenLoading}><Search size={17}/>{L("Keresés","Search")}</button></form>{cnenError&&<p className="question">{cnenError}</p>}{cnenLoading&&!cnenData&&<p className="cnen-loading"><LoaderCircle className="spin" size={17}/>{L("A tartalom betöltése…","Loading content…")}</p>}{cnenData&&<><div className="cnen-result-meta">{cnenData.query?L(`${cnenData.total} találat`,`${cnenData.total} results`):L("Tallózás a KN-áruosztályok között","Browse CN headings")}</div><div className="cnen-browser-grid"><div className="cnen-results" role="list">{cnenData.results?.map((item)=><button type="button" role="listitem" className={cnenSelected?.code===item.code?"selected":""} key={item.code} onClick={()=>loadCnenRecord(item.code)}><code>{groupedCnen(item.code)}</code><span>{item.heading}</span>{item.ruleTypes?.length>0&&<small>{item.ruleTypes.join(" · ")}</small>}</button>)}</div><article className="cnen-detail">{cnenSelected?<><div className="cnen-detail-title"><code>{groupedCnen(cnenSelected.code)}</code><h3>{cnenSelected.heading}</h3></div><div className="cnen-rule-tags">{cnenSelected.ruleTypes?.map((type)=><span key={type}>{type}</span>)}</div><p>{cnenSelected.content}</p><button type="button" className="cnen-children" onClick={()=>{setCnenQuery(cnenSelected.code);loadCnenSearch(cnenSelected.code);}}>{L("Kód alatti megjegyzések tallózása","Browse notes below this code")}</button></>:<p>{L("Válasszon egy KN-kódot a részletes magyarázathoz.","Select a CN code to view its explanatory note.")}</p>}</article></div></>}</section>}
