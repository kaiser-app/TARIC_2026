...
const groupedCnen = (value) => String(value || "").match(/.{1,2}/g)?.join(" ") || "—";
const measureSectionKey = (type) => ["103", "107"].includes(type) ? "customs" : type === "AAF" ? "tax" : "other";
...
const mergeConfirmedFacts = (current = {}, patch = {}) => ({
  ...current,
  ...patch,
  materials: [...new Set([...(current.materials || []), ...(patch.materials || [])])],
...
export default function App() {
  ...
  const [cnenQuery,setCnenQuery]=useState(""),[cnenData,setCnenData]=useState(null),[cnenSelected,setCnenSelected]=useState(null),[cnenLoading,setCnenLoading]=useState(false),[cnenError,setCnenError]=useState("");
  const [cnenPath,setCnenPath]=useState(null);
  const [refreshInfo,setRefreshInfo]=useState(null),[refreshLoading,setRefreshLoading]=useState(false),[refreshError,setRefreshError]=useState("");
  ...
  const runClassification = async (inputText = query, factState = confirmedFacts) => {
  ...
  const chooseClarification = (option) => {
    const appended = lang==="en"&&option.appendTextEn?option.appendTextEn:option.appendText;
    const next = `${query.trim()}${query.trim() ? ", " : ""}${appended}`;
    const nextFacts = mergeConfirmedFacts(confirmedFacts, option.confirmedFact || {});
    setQuery(next);
    setConfirmedFacts(nextFacts);
    runClassification(next,nextFacts);
  };
  ...
  const loadCnenRecord=async(codeValue)=>{
  ...
  useEffect(()=>{if(topPanel==="content"&&!cnenData&&!cnenLoading)loadCnenSearch("");},[topPanel]);
  useEffect(()=>{
    const codeValue=clean(cnenSelected?.code);
    if(!codeValue){setCnenPath(null);return;}
    let active=true;
    getJson(`/api/nomenclature-tree?path=${encodeURIComponent(codeValue)}`)
      .then((data)=>{if(active)setCnenPath(data);})
      .catch(()=>{if(active)setCnenPath(null);});
    return ()=>{active=false;};
  },[cnenSelected?.code]);
  const significantCnenCode=(value)=>{const d=clean(value);const trimmed=d.replace(/0+$/,"");const length=trimmed.length<=4?4:trimmed.length<=6?6:trimmed.length<=8?8:10;return d.slice(0,length);};
  const loadRefreshInfo=async()=>{setRefreshLoading(true);setRefreshError("");try{setRefreshInfo(await getJson("/api/refresh-data"));}catch(error){setRefreshInfo(null);setRefreshError(error.message);}finally{setRefreshLoading(false);}};
  const cnenRuleLabel=(type)=>lang==="hu"?({inclusion:"befoglalás",exclusion:"kizárás",definition:"meghatározás",cross_reference:"hivatkozás"}[type]||type):type;
  const cnenResultButton=(item)=><button type="button" role="listitem" className={`cnen-entry ${cnenSelected?.code===item.code?"selected":""}`} key={item.code} onClick={()=>loadCnenRecord(item.code)}><code>{groupedCnen(item.code)}</code><span>{lang==="hu"&&item.headingHu?item.headingHu:item.heading}</span>{lang==="hu"&&item.headingHu&&<small className="cnen-english-heading">EN: {item.heading}</small>}{item.ruleTypes?.length>0&&<small>{item.ruleTypes.map(cnenRuleLabel).join(" · ")}</small>}</button>;
  ...
  return <main>
    <nav><div className="brand"><ShieldCheck />TARIC 2026</div><div className="nav-actions"><button className={topPanel==="content"?"active":""} onClick={()=>setTopPanel(topPanel==="content"?null:"content")}>{L("Tallózás","Browse")}</button><button className={topPanel==="ai"?"active":""} onClick={()=>setTopPanel(topPanel==="ai"?null:"ai")}>AI</button><button onClick={()=>setTopPanel(topPanel==="integration"?null:"integration")}>{L("Integráció","Integration")}</button><button onClick={()=>setTopPanel(topPanel==="settings"?null:"settings")}>{L("Beállítások","Settings")}</button><button className={topPanel==="refresh"?"active":""} onClick={()=>{const next=topPanel==="refresh"?null:"refresh";setTopPanel(next);if(next==="refresh"&&!refreshInfo&&!refreshLoading)loadRefreshInfo();}}>{L("Frissítés","Update")}</button><div className="language"><button className={lang === "hu" ? "active" : ""} onClick={() => setLang("hu")}>HU</button><button className={lang === "en" ? "active" : ""} onClick={() => setLang("en")}>EN</button></div></div></nav>
    ...
    {topPanel==="ai"&&<section className="top-drawer ai-drawer"><h2>AI</h2><p className="ai-placeholder">{L("Ez a menüpont előkészítés alatt áll — a tartalma egy későbbi verzióban érkezik.","This section is under preparation — its content arrives in a later release.")}</p></section>}
    {topPanel==="refresh"&&<section className="top-drawer refresh-drawer"><h2>{L("Törzsadat-frissítés","Master data update")}</h2><p>{L("A NAV/OpenKKK publikálási mappában elérhető legfrissebb törzsadat-csomagok. A letöltött ZIP-ekből az import folyamat állítja elő az alkalmazási adatot.","Latest master data packages available in the NAV/OpenKKK publication folder. The import pipeline builds the application data from the downloaded ZIPs.")}</p>{refreshLoading&&<p className="cnen-loading"><LoaderCircle className="spin" size={17}/>{L("Csomagok keresése…","Looking up packages…")}</p>}{refreshError&&<p className="question">{refreshError}</p>}{refreshInfo?.packages?.length>0&&<div className="refresh-packages">{refreshInfo.packages.map((pkg)=><a className="refresh-package" key={pkg.kind} href={pkg.downloadUrl} target="_blank" rel="noreferrer" download><code>{pkg.kind.toUpperCase()}</code><span>{pkg.fileName}</span><em>{L("Adatnap","Data date")}: {pkg.dataDate}</em><ExternalLink size={15}/></a>)}</div>}{refreshInfo?.status==="ok"&&<small>{L("Ellenőrizve","Checked")}: {new Date(refreshInfo.checkedAt).toLocaleString(lang==="hu"?"hu-HU":"en-GB")}</small>}<div className="refresh-actions"><button type="button" onClick={loadRefreshInfo} disabled={refreshLoading}>{L("Újraellenőrzés","Check again")}</button></div></section>}
    <section className="hero"><div><p className="eyebrow">{t.eye}</p><h1>{t.title}</h1><p className="lead">{t.lead}</p></div><label className="code-card"><span>{t.code}</span><input className="taric-code-input" inputMode="numeric" autoComplete="off" maxLength={10} value={code} onChange={(event)=>{setCode(clean(event.target.value));setResult(null);setMeasures(null);setError("");}} placeholder="0000000000" aria-label={t.code}/><small>{grouped(code)} · HS / KN / TARIC</small></label></section>
    {topPanel==="content"&&<section className="top-drawer cnen-browser">
      <div className="cnen-browser-heading"><div><h2>{L("KN Magyarázó Megjegyzések","CN Explanatory Notes")}</h2><p>{L("Keresés KN-kód, hivatalos magyar megnevezés vagy az eredeti angol magyarázat alapján.","Search by CN code, official Hungarian heading or the original English explanatory text.")}</p></div>{cnenData&&<span>{Number(cnenData.recordCount).toLocaleString(lang==="hu"?"hu-HU":"en-GB")} {L("kódolt megjegyzés","coded notes")}</span>}</div>
      <form className="cnen-search" onSubmit={(event)=>{event.preventDefault();loadCnenSearch();}}><input value={cnenQuery} onChange={(event)=>setCnenQuery(event.target.value)} placeholder={L("KN-kód vagy kifejezés, pl. 4202, telefontok…","CN code or phrase, e.g. 4202, mobile phones…")}/><button type="submit" disabled={cnenLoading}><Search size={17}/>{L("Keresés","Search")}</button></form>
      {cnenError&&<p className="question">{cnenError}</p>}{cnenLoading&&!cnenData&&<p className="cnen-loading"><LoaderCircle className="spin" size={17}/>{L("A tartalom betöltése…","Loading content…")}</p>}
      {cnenData&&<><div className="cnen-result-meta">{cnenData.query?L(`${cnenData.total} találat`,`${cnenData.total} results`):L("Kétjegyű KN-fejezetek – a négyszámjegyű vámtarifaszámok lenyithatók","Two-digit CN chapters – expand to browse four-digit headings")}</div><div className="cnen-browser-grid">
        <div className="cnen-results" role="list">{cnenData.chapters?.length?<div className="cnen-chapters">{cnenData.chapters.map((chapter)=><details className="cnen-chapter" key={chapter.code}><summary><code>{chapter.code}</code><span>{lang==="hu"&&chapter.headingHu?chapter.headingHu:L(`KN ${chapter.code}. fejezet`,`CN chapter ${chapter.code}`)}</span><small>{chapter.count}</small></summary><div>{chapter.items.map((item)=>cnenResultButton(item))}</div></details>)}</div>:cnenData.results?.map((item)=>cnenResultButton(item))}</div>
        <article className="cnen-detail">{cnenSelected?<><div className="cnen-detail-title"><code>{groupedCnen(cnenSelected.code)}</code><h3>{lang==="hu"&&cnenSelected.headingHu?cnenSelected.headingHu:cnenSelected.heading}</h3></div><dl className="cnen-hierarchy">{cnenSelected.sectionCode&&<div><dt>{L("Áruosztály","Section")}</dt><dd><code>{cnenSelected.sectionCode}</code><span>{lang==="hu"?(cnenSelected.sectionDescriptionHu||cnenSelected.sectionDescriptionEn):(cnenSelected.sectionDescriptionEn||cnenSelected.sectionDescriptionHu)}</span></dd></div>}{cnenSelected.chapterCode&&<div><dt>{L("Árucsoport","Chapter")}</dt><dd><code>{cnenSelected.chapterCode}</code><span>{lang==="hu"?(cnenSelected.chapterDescriptionHu||cnenSelected.chapterDescriptionEn):(cnenSelected.chapterDescriptionEn||cnenSelected.chapterDescriptionHu)}</span></dd></div>}</dl>{cnenPath?.breadcrumb?.length>0&&<div className="cnen-breadcrumb" aria-label={L("Hierarchia-útvonal","Hierarchy path")}>{cnenPath.breadcrumb.map((step,i)=><React.Fragment key={`${step.code}-${step.depth}`}>{i>0&&<span className="crumb-sep">›</span>}<button type="button" className={`crumb ${i===cnenPath.breadcrumb.length-1?"current":""}`} onClick={()=>loadCnenRecord(significantCnenCode(step.code))} title={step.displayCode}>{step.description}{step.residualResolution&&<em>{step.residualResolution}</em>}</button></React.Fragment>)}</div>}{cnenPath?.contextTree?.length>0&&<div className="cnen-context-tree"><h4>{L("Vonalas bontás — kontextusfa","Indent structure — context tree")}</h4><div className="context-rows">{cnenPath.contextTree.map((row,i)=><button type="button" key={`${row.code}-${row.depth}-${i}`} className={`context-row ${row.role}`} onClick={()=>loadCnenRecord(significantCnenCode(row.code))}><span className="context-dashes">{row.dashes}</span><code>{row.displayCode}</code><span className="context-desc">{row.description}{row.residualResolution&&<em>{row.residualResolution}</em>}</span>{row.declarable&&<small>{L("bevallható","declarable")}</small>}</button>)}</div>{cnenPath.residualNote&&<p className="mas-note">{cnenPath.residualNote}</p>}</div>}{lang==="hu"&&cnenSelected.headingHu&&<p className="cnen-original-heading"><b>EN:</b> {cnenSelected.heading}</p>}<div className="cnen-rule-tags">{cnenSelected.ruleTypes?.map((type)=><span key={type}>{cnenRuleLabel(type)}</span>)}</div><h4>{L("Magyar magyarázó szöveg","Explanatory note")}</h4><p>{lang==="hu"?(cnenSelected.contentHu||cnenSelected.content):cnenSelected.content}</p><div className="cnen-detail-actions"><button type="button" className="cnen-use-code" onClick={()=>applyCnenSelection(cnenSelected)}>{L("Kód és magyar megnevezés átvétele","Use code and Hungarian product name")}</button><button type="button" className="cnen-children" onClick={()=>{setCnenQuery(cnenSelected.code);loadCnenSearch(cnenSelected.code);}}>{L("Kód alatti megjegyzések tallózása","Browse notes below this code")}</button></div></>:<p>{L("Nyisson le egy kétjegyű fejezetet, majd válasszon vámtarifaszámot.","Expand a two-digit chapter, then select a heading.")}</p>}</article>
      </div></>}</section>}
    <section className="panel agent-panel"><form className="agent-form" onSubmit={submit}>
      <label>{L("Termék neve *","Product name *")}<input value={product} onChange={(e) => {setProduct(e.target.value);setCode("");setResult(null);setMeasures(null);setConfirmedFacts({});}} placeholder={L("pl. lítium-ion akkumulátoros csavarbehajtó","e.g. lithium-ion cordless screwdriver")} /></label>
      <label>{L("Rövid leírás","Short description")}<textarea value={query} onChange={(e) => {setQuery(e.target.value);setCode("");setResult(null);setMeasures(null);setConfirmedFacts({});}} onInput={(e)=>{e.currentTarget.style.height="auto";e.currentTarget.style.height=`${Math.min(e.currentTarget.scrollHeight,180)}px`;}} placeholder={L("anyag, funkció, feldolgozottság, kiszerelés — minél pontosabb, annál biztosabb a besorolás","material, function, processing and packaging — more detail improves classification")} rows="1" /></label>
      ...
      <button className={`primary agent-submit ${loading ? "is-loading" : ""}`} disabled={loading || (clean(code).length<4 && !product.trim() && !query.trim())}>{loading ? <LoaderCircle className="spin" size={16} /> : <Search size={18} />}{progressLabel}</button>
    </form></section>
    {error && <section className="result error"><AlertCircle /><div><h2>Hiba</h2><p>{error}</p></div></section>}
    {result && <details className={`result collapsible-result ${result.status === "clarification" ? "needs-input" : ""}`}><summary>{result.status === "clarification" ? t.clarification : t.result}{result.code&&<span className="summary-code">{grouped(result.code)}</span>}</summary><div className="collapsible-body">{result.code && <div className="result-code">{grouped(result.code)} <span>{result.confidence}</span></div>}{result.clarification && <p className="question">{lang==="en"&&result.clarificationEn?result.clarificationEn:result.clarification}</p>}{!!result.clarificationOptions?.length && <div className="clarification-options">{result.clarificationOptions.map((option) => <button type="button" key={option.id} disabled={loading} onClick={() => chooseClarification(option)}>{lang==="en"&&option.labelEn?option.labelEn:option.label}</button>)}</div>}{result.reasoning && <p>{result.reasoning}</p>}{result.breadcrumb?.length>0&&<p className="result-breadcrumb">{result.breadcrumb.filter((step)=>!step.isChapter).map((step)=>step.description).join(" › ")}</p>}{!!result.path?.length && <ol className="path">{result.path.map((row, i) => <li key={`${row.code}-${row.line}-${i}`} style={{ marginLeft: `${Math.max(0, Number(row.line || 0)) * 18}px` }}><span className="path-dashes">{"─ ".repeat(Math.max(0, Number(row.line || 0)))}</span><code>{grouped(row.code)}</code><span>{row.description}{row.residualResolution&&<em className="mas-resolution">{row.residualResolution}</em>}</span></li>)}</ol>}{!!result.legalSources?.length && <div className="source-validation"><b>{L("Forrásellenőrzés","Source validation")}</b><span>{L("Aktuális KN/TARIC nómenklatúra","Current CN/TARIC nomenclature")}</span>{result.cnenEvidence?.length > 0 && <span>{L(`KN Magyarázat · ${result.sourceValidation?.explanatoryNoteDate}${result.sourceValidation?.mappedOlderCode ? " · korábbi kódról megfeleltetve" : ""}`,`CN Explanatory Notes · ${result.sourceValidation?.explanatoryNoteDate}${result.sourceValidation?.mappedOlderCode ? " · mapped from an earlier code" : ""}`)}</span>}</div>}{result.dataDate && <small>Adatnap: {result.dataDate}</small>}</div></details>}
    ...
    {measures && <details className="result collapsible-result"><summary>{t.measures} <span className="count">{displayedMeasureGroups.length}</span></summary><div className="collapsible-body">
      {measures.valueCheck && <p className="measure-context">Származás: <b>{measures.origin||"CN"}</b> · Vámérték: <b>{measures.valueCheck.valueHuf?.toLocaleString("hu-HU")||"—"} Ft</b> ({measures.valueCheck.valueEur??"—"} EUR) · Kisértékű: <b>{measures.valueCheck.lowValueEligible?"igen":"nem"}</b></p>}
      {displayedMeasureGroups.length ? <div className="measure-groups">{displayedMeasureGroups.flatMap((group,i) => {
        const sectionKey = measureSectionKey(group.type);
        const previousSectionKey = i > 0 ? measureSectionKey(displayedMeasureGroups[i - 1].type) : null;
        const sectionTitle = sectionKey === "customs" ? L("Vámtételek", "Customs duties") : sectionKey === "tax" ? L("Adók", "Taxes") : L("Egyéb intézkedések", "Other measures");
        return [
          sectionKey !== previousSectionKey && <div className="measure-section-title" key={`section-${sectionKey}-${i}`}>{sectionTitle}</div>,
          <details className="measure-group" key={`${group.type}-${group.area}-${group.additionalCode}-${i}`}><summary><b>{group.type}</b><span>{group.label}</span><em>{group.area||"Erga omnes"}</em><div className="measure-badges">{group.variants?.length>1&&<small>{group.variants.length} {L("verzió","versions")}</small>}{group.conditionCount>0&&<small>{group.conditionCount} feltétel</small>}</div></summary>{group.variants?.length>1?<div className="variant-list">{group.variants.map((variant,j)=><div key={`${variant.area}-${variant.additionalCode}-${j}`}><code>{variant.additionalCodes?.join(", ")||variant.additionalCode||"—"}</code><span>{L("Kiegészítő kód","Additional code")}</span><em>{variant.rates?.map((rate)=>rate.expression||rate.value).filter(Boolean).join(", ")||variant.area||"Erga omnes"}</em>{variant.conditions?.length>0&&<div className="variant-conditions">{variant.conditions.map((condition,k)=><small key={`${condition.certificate}-${k}`}>{condition.certificate||"—"}: {condition.description||"Feltételhez kötött intézkedés"}</small>)}</div>}</div>)}</div>:group.conditions.length>0?<div className="condition-list">{group.conditions.map((condition,j)=><div key={`${condition.certificate}-${j}`}><code>{condition.certificate||"—"}</code><span>{condition.description||"Feltételhez kötött intézkedés"}</span></div>)}</div>:<p>Nincs külön igazolási feltétel.</p>}</details>,
        ];
      })}</div> : <p>{t.none}</p>}
      {measures.rawConditionCount>measures.count&&<small>{measures.rawConditionCount} technikai feltételsor {measures.count} intézkedéscsoportba rendezve.</small>}
    </div></details>}
    <section className="grid"><article className="notice"><ShieldCheck /><div><h2>{t.status}</h2><p>{t.text}</p><p><b>{L("Adatnap","Data date")}: {health?.dataVersion || L("betöltés…","loading…")}</b></p></div></article><article><h2>{t.sources}</h2><div className="links">{links.map((x) => {const href=x.label==="EU TARIC"?buildEuTaricUrl({code,date:options.date,lang}):x.href;return <a href={href} target="_blank" rel="noreferrer" key={x.label}>{x.label}<ExternalLink size={15} /></a>;})}</div></article><article><FileText /><h2>{t.report}</h2><p>{Number(health?.nomenclatureRows || 0).toLocaleString(lang==="hu"?"hu-HU":"en-GB")} {L("nómenklatúra-sor","nomenclature rows")} · {Number(health?.measures || 0).toLocaleString(lang==="hu"?"hu-HU":"en-GB")} {L("intézkedés","measures")} · 92 {L("AIS ellenőrzési szabály","AIS validation rules")}</p></article></section>
